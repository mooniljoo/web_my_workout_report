...
export default function routines(state = initialState, action) {
  switch (action.type) {
    ...
    case REMOVE_WORKOUT:
      ...
      console.log("\tstate", state);
      //복사해서 나와야 됨 변형시키면 안됨
      const new_state_remove = state[0].routine.workoutItems.filter(workoutItem => workoutItem.workoutItemName !== action.workoutItemNameText)
    ...
    case ADD_SET:
      ...
      const matchedWorkoutItems =state[0].routine.workoutItems.filter(workoutItem => workoutItem.workoutItemName === action.workoutItemNameText)
